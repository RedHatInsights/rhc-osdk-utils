...
func (r *ResourceList) GetByGVKAndNamespace(ctx context.Context, pClient client.Client, namespace string, gvk schema.GroupVersionKind) error {
...
func (r *ResourceCounter) Count(ctx context.Context, pClient client.Client) (ResourceCounterResults, error) {
	for _, namespace := range r.Query.Namespaces {
		resourceList, err := r.GetResourceList(ctx, pClient, namespace)
		if err != nil {
			return ResourceCounterResults{}, fmt.Errorf("couldn't get resource list: %w", err)
		}
...
func (r *ResourceCounter) GetResourceList(ctx context.Context, pClient client.Client, namespace string) (ResourceList, error) {
	resources := ResourceList{}
	err := resources.GetByGVKAndNamespace(ctx, pClient, namespace, r.Query.GVK)
	if err != nil {
		return resources, fmt.Errorf("could not get by GVK/Namespace: %w", err)
	}
